...
DESCRIPTION="Software that delivers parallel interactive visualizations"
...
SLOT="0"
KEYWORDS="~amd64 ~x86"
IUSE="cgns debug hdf5 mpi netcdf silo tcmalloc test threads xdmf2"
...
RDEPEND="${PYTHON_DEPS}
	...
	>=sci-libs/vtk-8.1.0[imaging,mpi=,python,rendering,qt5,xdmf2?,${PYTHON_USEDEP}]
	sys-libs/zlib
	x11-libs/qwt:6[qt5(+)]
	cgns? ( sci-libs/cgnslib )
	hdf5? ( sci-libs/hdf5 )
	netcdf? ( sci-libs/netcdf )
	silo? ( sci-libs/silo[hdf5?] )
...
PATCHES=(
	"${FILESDIR}/${PN}-2.12.3-findpython.patch"
	"${FILESDIR}/${PN}-2.12.3-findsilo.patch"
	"${FILESDIR}/${P}-vtklibs.patch"
	"${FILESDIR}/${PN}-2.12.3-dont_symlink_visit_dir.patch"
	"${FILESDIR}/${PN}-2.12.3-cmakelist.patch"
	"${FILESDIR}/${P}-qwt-pkgconfig.patch"
)

src_prepare() {
	cmake-utils_src_prepare
	use mpi && eapply "${FILESDIR}/${PN}-2.12.3-vtkmpi.patch"
